...
const readSeen = () => {
  try {
    return new Set<string>(JSON.parse(sessionStorage.getItem("customli_display_seen_ready") || "[]"));
  } catch {
...
export default function PosDisplay() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        sessionStorage.setItem("customli_display_seen_ready", JSON.stringify(Array.from(seen.current)));
        setTickets(next);
        setError("");
      } catch (cause: any) {
  ...
  return (
    <main className="min-h-dvh bg-zinc-950 p-6 text-center text-white">
      <p className="font-bold tracking-[.3em] text-yellow-400">CUSTOMLI POS</p>

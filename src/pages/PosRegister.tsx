...
export default function PosRegister() {
  ...
  const refreshOrderNumber = () => fetch("/api/pos/orders/next-ticket",{credentials:"include"})
    .then(r => { if (r.status === 401) { window.location.assign("/register?lock=1"); return null; } return r.json(); })
  ...
  const codeDiscountPreview = selectedDiscountData ? Math.min(subtotal, selectedDiscountData.discount_type === "percent" ? subtotal * selectedDiscountData.value / 100 : selectedDiscountData.value) : 0;
  const grabDiscountPreview = mode === "grab" ? Math.min(subtotal, Math.max(0,Number(grabDiscount || 0))) : 0;
  const discountPreview = mode === "grab" ? grabDiscountPreview : codeDiscountPreview;
  ...
  const startItem = async (item:Item) => {
    ...
    try {
      ...
      if (response.status === 401) { window.location.assign("/register?lock=1"); return; }
      const body = await response.json();
  ...
  return <main className="h-dvh overflow-hidden bg-white text-[#171717]">
    <header className="flex h-[70px] items-center justify-between bg-[#111111] px-5 text-white shadow-lg">
      <div className="flex items-center gap-4">
        <img src="/customli-mark.svg" alt="Customli POS" className="h-12 w-12 rounded-xl object-contain"/>
        <nav className="hidden gap-1 lg:flex"><span className="rounded-xl bg-white/10 px-4 py-2 text-sm font-semibold">{ui.pos}</span><a href="/kitchen" className="rounded-xl px-4 py-2 text-sm text-zinc-300 hover:bg-white/10">{ui.kitchen}</a><a href="/display" className="rounded-xl px-4 py-2 text-sm text-zinc-300 hover:bg-white/10">{ui.display}</a><a href="/shifts" className="rounded-xl px-4 py-2 text-sm text-zinc-300 hover:bg-white/10">{ui.shift}</a><a href="/" className="rounded-xl px-4 py-2 text-sm font-semibold text-[#ffd400] hover:bg-white/10">Dashboard</a></nav>
      </div>
